...
def chart_data():
    if not get_authenticated_user():
        return error_response("Telegram authentication required.", 401)
    return jsonify(market.get_candles()), 200
...
def market_state():
    ...
    return success_response({
        ...
        "socket_connected": state.get("socket_connected", False),
...
def trade_history():
    ...
    try:
        ...
        limit = 20
    return success_response({"trades": trading.get_trade_history(user["id"], limit)})
